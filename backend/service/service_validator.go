...
func (service *ValidatorService) GetValidators(typ, origin string, page, size int, istotal bool) interface{} {
	if origin == "browser" {
		...
		for i, v := range validatorList {
			...
			validator.VotingRate = float32(v.VotingPower) / float32(totalVotingPower) / lcd.NativeTokenPrecision
...
func (service *ValidatorService) GetValidatorDetail(validatorAddr string) vo.ValidatorForDetail {
	...
	res := vo.ValidatorForDetail{
		TotalPower:              totalVotingPower / lcd.NativeTokenPrecision,
		SelfPower:               validatorAsDoc.VotingPower / lcd.NativeTokenPrecision,
