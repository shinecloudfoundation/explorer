	...
	for _, v := range data {
		...
		for _, m := range v.Msgs {
			...
			case types.TxTypeWithdrawDelegatorReward:
				...
				break
			case types.TxTypeWithdrawDelegatorRewards:
				msgVO := msgvo.TxMsgWithdrawDelegatorRewards{}
				if err := msgVO.BuildMsgByUnmarshalJson(utils.MarshalJsonIgnoreErr(m.MsgData)); err != nil {
					logger.Error("Build TxMsgWithdrawDelegatorRewards ByUnmarshalJson", logger.String("err", err.Error()))
				} else {
					msgDataVO = msgVO
				}
				break
			case types.TxTypeWithdrawValidatorRewards:
				msgVO := msgvo.TxMsgWithdrawValidatorRewards{}
				if err := msgVO.BuildMsgByUnmarshalJson(utils.MarshalJsonIgnoreErr(m.MsgData)); err != nil {
					logger.Error("Build TxMsgWithdrawValidatorRewards ByUnmarshalJson", logger.String("err", err.Error()))
				} else {
					msgDataVO = msgVO
				}
				break
	...
	for i := 0; i < len(items); i++ {
		if stakeTx, ok := items[i].(vo.StakeTx); ok {
			for _, msg := range stakeTx.Msgs {
				...
				case types.TxTypeWithdrawDelegatorRewards, types.TxTypeWithdrawValidatorRewards:
					if stakeTx.Status == types.Success {
						...
						if len(stakeTx.Msgs) > 0 {
							...
							case types.TxTypeWithdrawValidatorRewards:
								msgData := stakeTx.Msgs[0].MsgData.(msgvo.TxMsgWithdrawValidatorRewards)
								Tmoniker := ""
