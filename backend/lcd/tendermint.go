...
func NodeInfo(lcdurl string) (result NodeInfoVo, err error) {
	...
	var response NodeInfoVoReponse
	if err := json.Unmarshal(resBytes, &response); err != nil {
		logger.Error("get node_info error", logger.String("err", err.Error()))
		return result, err
	}
	return response.NodeInfoVo, nil
}
...
func ValidatorSet(height int64) (result ValidatorSetVo) {
	...
	var response ValidatorSetVoResponse
	if err := json.Unmarshal(resBytes, &response); err != nil {
		logger.Error("BlockLatest error", logger.String("err", err.Error()))
		return result
	}
