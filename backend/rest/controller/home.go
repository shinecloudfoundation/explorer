...
func registerNavigationBar(r *mux.Router) error {
	doApi(r, types.UrlRegisterNavigationBar, "GET", func(request vo.IrisReq) interface{} {
		...
		var queryBondedInfo = func() {
			...
			totalBalanceOfNativeTokenResult, err := lcd.GetTokenTotalSupply(lcd.NativeTokenDenom)
			if err != nil {
				panic(err)
			}
			totalBalanceOfNativeToken, err := strconv.Atoi(totalBalanceOfNativeTokenResult.Amount)
			if err != nil {
				panic(err)
			}

			var poolStake = lcd.StakePool()
			bondedAmount, err := strconv.Atoi(poolStake.BondedTokens)
			if err != nil {
				panic(err)
			}
			result.BondedTokens = strconv.Itoa(bondedAmount / lcd.NativeTokenPrecision)

			bondedRatio := float64(bondedAmount) / float64(totalBalanceOfNativeToken)
			bondedRatioStr := strconv.FormatFloat(bondedRatio, 'f', -1, 64)

			result.BondedRatio = bondedRatioStr
			result.TotalSupply = strconv.Itoa(totalBalanceOfNativeToken / lcd.NativeTokenPrecision)
		}
		...
		var getTokenStatCirculation = func() {
			...
			balanceOfMintAccount, err := lcd.GetBalanceOfAddr(lcd.MintedTokenLockedAddr)
			if err != nil {
				panic(err)
			}

			var balanceOfNativeToken int
			for _, balance := range balanceOfMintAccount {
				if balance.Denom == lcd.NativeTokenDenom {
					balanceOfNativeToken, err = strconv.Atoi(balance.Amount)
					if err != nil {
						panic(err)
					}
				}
			...
			result.Circulation = strconv.Itoa((totalBalanceOfNativeToken - balanceOfNativeToken)/ lcd.NativeTokenPrecision)
		}
