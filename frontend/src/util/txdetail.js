...
export default class formatMsgsAndTags {
    static switchTxType(dataTx){
        ...
        switch (dataTx.type) {
            ...
            case Constant.TxType.BEGINUNBONDING :
                ...
                break;
            case Constant.TxType.WITHDRAWDELEGATORREWARDS :
                return message = formatMsgsAndTags.txTypeWithdrawDelegatorRewards(dataTx,Constant.TxType.WITHDRAWDELEGATORREWARDS);
                break;
            case Constant.TxType.WITHDRAWVALIDATORREWARDS :
                return message = formatMsgsAndTags.txTypeWithdrawValidatorRewards(dataTx,Constant.TxType.WITHDRAWVALIDATORREWARDS);
                break;
    ...
    static txTypeBeginRedelegate(dataTx,txType){
        ...
        if(dataTx.status === 'success'){
            if(dataTx.tags){
                ...
                message[Constant.TRANSACTIONMESSAGENAME.AMOUNT].unshift(`${amountObj.amountNumber} ${amountObj.tokenName}`);
                message[Constant.TRANSACTIONMESSAGENAME.FROM].unshift(dataTx.tags['source_validator']);
                message[Constant.TRANSACTIONMESSAGENAME.SHARES].unshift(`${Tools.numberMoveDecimal(dataTx.tags['amount'])} SHARES`);
                message[Constant.TRANSACTIONMESSAGENAME.TO].unshift(dataTx.tags['destination_validator'])
                message[Constant.TRANSACTIONMESSAGENAME.TOSHARES].unshift(`${Tools.numberMoveDecimal(dataTx.tags['amount'])} SHARES`);
                message[Constant.TRANSACTIONMESSAGENAME.ENDTIME].unshift(Tools.format2UTC(dataTx.tags['completion_time']));
            }
        }else {
            if(dataTx.msgs && Array.isArray(dataTx.msgs) && dataTx.msgs !== null){
                dataTx.msgs.forEach( item => {
    ...
    static txTypeBeginUnbonding(dataTx,txType){
        ...
        if(dataTx.status === 'success'){
            if(dataTx.tags){
                ...
                message[Constant.TRANSACTIONMESSAGENAME.FROM].unshift(dataTx.tags['validator']);
                message[Constant.TRANSACTIONMESSAGENAME.AMOUNT].unshift(`${amountObj.amountNumber} ${amountObj.tokenName}`);
                message[Constant.TRANSACTIONMESSAGENAME.TO].unshift(dataTx.tags.recipient);
                message[Constant.TRANSACTIONMESSAGENAME.ENDTIME].unshift(Tools.format2UTC(dataTx.tags['completion_time']))
            }
            if(dataTx.msgs){
                if(dataTx.msgs && Array.isArray(dataTx.msgs)&& dataTx.msgs !== null){
            ...
            if(dataTx.tags && dataTx.tags.amount && dataTx.tags['completion_time']){
                amountObj = Tools.formatListByTagsBalance(dataTx.tags);
                message[Constant.TRANSACTIONMESSAGENAME.AMOUNT].unshift(`${amountObj.amountNumber} ${amountObj.tokenName}`);
                message[Constant.TRANSACTIONMESSAGENAME.ENDTIME].unshift(Tools.format2UTC(dataTx.tags['completion_time']))
    ...
    static txTypeWithdrawDelegatorRewards(dataTx,txType){
        let message = {},amountObj;
        message[Constant.TRANSACTIONMESSAGENAME.TXTYPE] = [];
        message[Constant.TRANSACTIONMESSAGENAME.FROM] = [];
        message[Constant.TRANSACTIONMESSAGENAME.AMOUNT] = [];
    ...
    static txTypeWithdrawValidatorRewards(dataTx,txType){
        let message = {},amountObj;
        message[Constant.TRANSACTIONMESSAGENAME.TXTYPE] = [];
        message[Constant.TRANSACTIONMESSAGENAME.FROM] = [];
        message[Constant.TRANSACTIONMESSAGENAME.AMOUNT] = [];
            ...
            dataTx.msgs.forEach( item => {
                switch (item.type) {
                    ...
                    case Constant.SUBMITPROPOSALTYPE.SUBMITSOFTWAREUPGRADEPROPOSAL:
                        ...
                        message[Constant.TRANSACTIONMESSAGENAME.THRESHOLD] = [];
                        if(item.msg){
                            ...
                            message[Constant.TRANSACTIONMESSAGENAME.THRESHOLD].unshift(`${Tools.formatPercentage(item.msg.threshold)} %`);
                        }
                        break;
