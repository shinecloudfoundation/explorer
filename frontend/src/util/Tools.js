...
export default class Tools{
    ...
    static utc2beijing(utc_datetime) {
        ...
        var beijing_datetime = new Date(parseInt(timestamp) * 1000).toLocaleString().replace(/上午/g, " ").replace(/下午/g, " ")+' GMT+8';
	...
	static format2UTC(originTime){
		return this.utc2beijing(originTime);
	}
	/**
	 * 后端返回的数据转换成标准格式 ()
	 */
	...
	static formatNumber(num){
		return new BigNumber(num).div(1000000).toNumber();
	}
	/**
	 * 格式化数字并保留两位数(可以废除)
	 *
	...
	/**
	 * 格式化数字类型是string的数字并让小数点左移6位 (本质是移动小数点的位置)
	 *
	 */
	...
	static formatPriceToFixed(value,fixedValue){
		return new BigNumber(value).div(1000000).toFormat(fixedValue)
	}
	/**
	...
	static formatFeeToFixedNumber(num){
		return  Tools.toFixedformatNumber(Tools.formatNumber(num) ,6);
	}
	...
	static formatDenom(denom){
		if(denom.toLowerCase() === "uscds" || denom.toLowerCase() === "scds"){
			return "SCDS"
		}else {
			return denom
		}
	...
	static addressRoute(address) {
		if(address) {
			if (address.substring(0, 13) === "scloudvaloper") {
	...
	* */
	...
	let diffValue = 1,defaultValue = 6;
	if(param){
  ...
  static FormatUptime(number) {
			...
			switch (data.type) {
				...
				case Constant.TxType.BEGINREDELEGATE:
					...
					if(data.status === 'success'){
					    if(data.tags){
                            fromAddressAndMoniker.unshift(Tools.getFromAndToMoniker(data.tags['source_validator'],data.monikers));
                            toAddressAndMoniker.unshift(Tools.getFromAndToMoniker(data.tags['destination_validator'],data.monikers))
                        }
                    }else {
					    if(data.msgs){
                            data.msgs.forEach( item => {
				...
				case Constant.TxType.BEGINUNBONDING:
					...
					if(data.status === 'success'){
					    if(data.tags){
                            fromAddressAndMoniker.unshift(Tools.getFromAndToMoniker(data.tags['validator'],data.monikers));
                            toAddressAndMoniker.unshift(Tools.getFromAndToMoniker(data.tags['recipient'],data.monikers))
                        }
                    }else {
					    if(data.msgs){
					        data.msgs.forEach( item => {
	...
	* */
	...
	static formatListByTagsBalance(tags,flSplitNum){
		let [amountNumber,tokenName] = ['--','--'];
		if(tags && tags.amount){
			let tokenValue = Tools.formatAccountCoinsAmount(tags.amount);
			let tokenStr = String(Tools.numberMoveDecimal(tokenValue,6));
			if(flSplitNum){
    ...
    static formatFee(Fee){
        if(Fee.amount && Fee.denom){
            return `${Tools.formatStringToFixedNumber(String(Tools.formatNumber(Fee.amount)),6)} ${Tools.formatDenom(Fee.denom).toUpperCase()}`;
        }
    }
    /**
     * 格式化交易详情页的amount
