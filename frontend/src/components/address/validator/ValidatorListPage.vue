<template>
    <div class="blocks_list_page_wrap">
        <page-title :title="convertTitle(pageTitle)"
                    :content="generateContent()"></page-title>
            ...
            <div :class="blocksListPageWrap" style="margin-bottom:0;">
                <div class="validators_status_tab">
                    <div style="height: 0.3rem; margin-top: 0.2rem;">
                        <m-tabs :data="convertValidatorStatusTitleList(validatorStatusTitleList)" :chose="selectValidatorStatus"></m-tabs>
                    </div>
                </div>
            </div>
        </div>
...
<script>
	...
	export default {
		...
		methods: {
		    convertValidatorStatusTitleList(valStatusTitleList){
                let newValStatusTitleList = [];
                valStatusTitleList.forEach(element => {
                    let statusTitle = Object.assign({}, element);
                    let str = "staking." + statusTitle.title;
                    statusTitle.title = this.$t(str);
                    newValStatusTitleList.push(statusTitle);
                });
                return newValStatusTitleList;
            },
		    generateContent(){
                let str = "staking." + this.validatorPageTitle;
                return this.count.toString()+" "+this.$t(str)
            },
            convertTitle(title){
                let str = "staking." + title;
                return this.$t(str);
            },
