<template>
    <div class="address_information_container">
        <page-title :title="convertTitle(pageTitle)"
        ...
        <div class="address_information_content">
            <div class="address_information_assets_container">
                <div class="address_information_assets_title">{{$t('delegator.Assets.Name')}}</div>
            ...
            <div class="address_information_delegation_tx_container">
                <div class="address_information_delegation_tx_content">
                    <div class="address_information_delegation_title">{{$t('delegator.Delegations.Name')}}
                ...
                <div class="address_information_unbonding_delegation_tx_content">
                    <div class="address_information_unbonding_delegation_title">{{$t('delegator.UnbondingDelegations.Name')}}
            ...
            <div class="address_information_redelegation_header_title">{{$t('delegator.DelegatorRewards.Name')}}
            ...
            <div class="address_information_redelegation_tx_container">
                <div class="address_information_delegator_rewards_content">
                    <div class="address_information_detail_option">
                        <span class="address_information_detail_option_name">{{$t('delegator.DelegatorRewards.WithdrawTo')}}:</span>
                ...
                <div class="address_information_detail_container" :class="OperatorAddress !== '--' ? '' :'hide_style'" :style="{visibility:OperatorAddress && OperatorAddress !== '--' ? 'visible':'hidden'}">
                    <div class="address_information_redelegation_title">{{$t('delegator.ValidatorRewards.Name')}}
                    ...
                    <ul class="address_information_detail_content">
                        <li class="address_information_detail_option">
                            <span class="address_information_detail_option_name">{{$t('delegator.ValidatorRewards.ValidatorMoniker')}}:</span>
                            <div class="validator_status_content">
                                ...
                                <span class="address_information_address_status_active" v-if="validatorStatus === 'Active'">{{$t('delegator.ValidatorRewards.Active')}}</span>
                                <span class="address_information_address_status_candidate" v-if="validatorStatus === 'Candidate'">{{$t('delegator.ValidatorRewards.Candidate')}}</span>
                                <span class="address_information_address_status_jailed" v-if="validatorStatus === 'Jailed'">{{$t('delegator.ValidatorRewards.Jailed')}}</span>
                            </div>
                        </li>
                        <li class="address_information_detail_option" style="margin-top: 0.05rem">
                            <span class="address_information_detail_option_name">{{$t('delegator.ValidatorRewards.OperatorAddress')}}:</span>
                            <span class="address_information_detail_option_value">
            ...
            <div class="address_information_transaction_container">
                <div class="address_information_transaction_header_content">
                    <span class="address_information_transaction_title">{{allTxCountNum}} {{$t('delegator.Transactions.Txs')}}</span>
                    <div class="address_information_list_filter_content">
                        <div class="filter_content">
                            <div class="tx_type_content">
                                <div class="tx_type_mobile_content">
                                    <el-cascader v-model="value"
                                                 :options="convertTxTypeOption(txTypeOption)"
                                    ...
                                    <el-select v-model="statusValue" :change="filterTxByStatus(statusValue)">
                                        <el-option v-for="(item, index) in status"
                                                   :key="index"
                                                   :label="convertStatus(item.label)"
                                ...
                                <div class="tx_type_mobile_content">
                                    <el-date-picker  type="date"
                                                     ...
                                                     :placeholder="$t('delegator.Transactions.SelectDate')">
                                    </el-date-picker>
                                ...
                                <div class="tx_type_mobile_content">
                                    <div class="search_btn" @click="getFilterTxs">{{$t('delegator.Transactions.Search')}}</div>
                ...
                <div class="address_information_list_content">
                    <div>
                        <m-address-information-table
                                :items="convertTxList(transactionsItems)"
        ...
        methods:{
            ...
            convertTxList(txList){
                ...
                txList.forEach(element => {
                    ...
                    tx.txType = this.convertTxType(tx.txType);
                    tx.status = this.convertStatus(tx.status);
                    newTxList.push(tx);
                });
                return newTxList;
            },
            convertTxTypeOption(txTypeOption){
                let newTxTypeOption = [];
                txTypeOption.forEach(element => {
                    let children = [];
                    if (Array.isArray(element.children) && element.children.length > 0){
                        element.children.forEach(ele => {
                            let child = {
                                value: ele.value,
                                label:this.convertTxType(ele.label)
                            };
                            children.push(child);
                        })
                    }
                    ...
                    if (Array.isArray(element.children)) {
                        txType = {
                            value:element.value,
                            label:this.convertTxType(element.label),
                            slot:element.slot,
                            children: children
                        };
                    } else {
                    ...
                    newTxTypeOption.push(txType);
                });
                return newTxTypeOption;
            },
	            ...
	            this.assetsItems = this.assetList.map( item => {
	            	if(item.denom === 'uscds'){
                        return {
                            ...
                            balance: item.amount ? `${Tools.formatNumber(item.amount).toFixed(this.fixedNumber)} SCDS`: 0,
                            delegatedValue: this.totalDelegator ? this.totalDelegator : 0,
                            delegated: this.totalDelegator ? `${Tools.formatStringToFixedNumber(new BigNumber(moveDecimal(Tools.formatNumber(this.totalDelegator).toString(),-2)).toFormat(),this.fixedNumber)} ${Constant.Denom.IRIS.toUpperCase()}`: 0,
                            unBondingValue: this.totalUnBondingDelegator ? this.totalUnBondingDelegator : 0,
                            unBonding: this.totalUnBondingDelegator ?`${Tools.formatStringToFixedNumber(new BigNumber(moveDecimal(Tools.formatNumber(this.totalUnBondingDelegator).toString(),-2)).toFormat(),this.fixedNumber)} ${Constant.Denom.IRIS.toUpperCase()}`  : 0,
                            reward: this.allRewardsValue ? this.allRewardsValue : 0,
                            totalAmount:`${Tools.formatStringToFixedNumber(new BigNumber(moveDecimal((
                                Number(Tools.formatStringToFixedNumber(Tools.formatNumber(item.amount).toString(),this.fixedNumber))*100 +
                                Number(Tools.formatStringToFixedNumber(Tools.formatNumber(this.totalDelegator).toString(),this.fixedNumber)) +
                                Number(Tools.formatStringToFixedNumber(Tools.formatNumber(this.totalUnBondingDelegator).toString(),this.fixedNumber))+
                                Number(Tools.formatStringToFixedNumber(Tools.formatNumber(this.allRewardsAmountValue).toString(),this.fixedNumber)) * 100
                            ).toString(),-2)).toFixed(this.fixedNumber),this.fixedNumber)} ${Constant.Denom.IRIS.toUpperCase()}` ,
                        }
                    }else {
			            return {
				            token: item.denom,
	            ...
	            Server.commonInterface({delegationList:{
		            ...
		            try {
		            	if(res && res.length > 0){
                            ...
                            this.totalDelegatorValue = `${Tools.formatStringToFixedNumber(new BigNumber(moveDecimal(Tools.formatNumber(this.totalDelegator).toString(),-2)).toFormat(),this.fixedNumber)} ${Constant.Denom.IRIS.toUpperCase()}`
                        }else {
	            ...
	            Server.commonInterface({unDelegationList:{
		            ...
		            try {
		            	if(res && res.length > 0){
				            ...
				            this.totalUnBondingDelegatorValue = `${Tools.formatStringToFixedNumber(new BigNumber(moveDecimal(Tools.formatNumber(this.totalUnBondingDelegator).toString(),-2)).toFormat(),this.fixedNumber)} ${Constant.Denom.IRIS.toUpperCase()}`
	            ...
	            Server.commonInterface({rewardList:{
		            ...
		            try {
		            	if(res && res.delagations_rewards && res.delagations_rewards.length > 0) {
                            ...
                            if(res.delagations_rewards.length > 0){
                                res.delagations_rewards.forEach( item => {
                                    if(item.amount && item.amount.length > 0){
                                        item.amount[0].amount = (Tools.formatStringToFixedNumber(Tools.numberMoveDecimal(item.amount[0].amount,-6),this.fixedNumber)) * 100
                                    }
                                })
                            ...
                            this.allRewardsAmountValue = res.total_rewards ? Tools.formatStringToFixedNumber(res.total_rewards[0].amount,this.fixedNumber) : 0;
					        ...
					        this.transactionsItems = res.Data.map( item => {
						        ...
						        if(item.type === 'BeginUnbonding' || item.type === 'BeginRedelegate'){
							        if(item.status === 'success'){
								        if(item.tags.amount){
									        let tokenValue = Tools.formatAccountCoinsAmount(item.tags.amount);
									        let tokenStr = String(Tools.numberMoveDecimal(tokenValue[0],6));
									        item.amount[0].formatAmount =  Tools.formatStringToFixedNumber(tokenStr,2);
									        Amount = item.amount.map(listItem => `${listItem.formatAmount} SCDS`).join(',');
                                        }
							        }
						        }else {
						            if(item.amount && item.amount.length > 0){
	        	...
	        	if(this.flDelegationNextPage){
			        this.delegationsItems = this.delegationPageNationArrayData[pageNum].map(item => {
				        return {
					        ...
					        shares: `${Tools.formatStringToFixedNumber(new BigNumber(moveDecimal(item.shares,(Number(this.fixedNumber)* -1))).toFormat(),this.fixedNumber)} SHARE`,
					        block: item.height,
