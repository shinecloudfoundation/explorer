<template>
    <div class="address_detail_table" style="background:#fff;min-height: 2.34rem;">
        <m-table :columns="convertFields(fields)" :data="items" :width="width">
        ...
        methods:{
            convertRewards(rewards){
                ...
                rewards.forEach(element => {
                    let reward = Object.assign({}, element);
                    let str = "delegator.DelegatorRewards." + reward.title.replace(/\s/g, '');
                    reward.title = this.$t(str);
            ...
            convertAssets(assets){
                ...
                assets.forEach(element => {
                    let asset = Object.assign({}, element);
                    let str = "delegator.Assets." + asset.title.replace(/\s/g, '');
                    asset.title = this.$t(str);
                    newAssets.push(asset);
                });
                return newAssets;
            },
            convertUnBondingDelegations(unBondingDelegations){
                let newUnBondingDelegations = [];
                unBondingDelegations.forEach(element => {
                    let unBondingDelegation = Object.assign({}, element);
                    let str = "delegator.UnbondingDelegations." + unBondingDelegation.title.replace(/\s/g, '');
                    unBondingDelegation.title = this.$t(str);
                    newUnBondingDelegations.push(unBondingDelegation);
                });
                return newUnBondingDelegations;
            },
            convertDelegations(delegations){
                let newDelegations = [];
                delegations.forEach(element => {
                    let delegation = Object.assign({}, element);
                    let str = "delegator.Delegations." + delegation.title.replace(/\s/g, '');
                    delegation.title = this.$t(str);
            ...
            convertTransactions(transactions){
                ...
                transactions.forEach(element => {
                    let transaction = Object.assign({}, element);
                    let str = "delegator.Transactions." + transaction.title.replace(/\s/g, '');
                    transaction.title = this.$t(str);
                    newTransactions.push(transaction);
                });
                return newTransactions;
            },
            convertFields(fields){
                if (this.listName === 'rewards'){
                    return this.convertRewards(fields);
                } else if (this.listName === 'assets') {
                    return this.convertAssets(fields)
                } else if(this.listName === 'delegations') {
                    return this.convertDelegations(fields);
                } else if(this.listName === 'unBondingDelegations') {
                    return this.convertUnBondingDelegations(fields);
                } else if(this.listName === 'transactions') {
                    return this.convertTransactions(fields);
                }
                return fields;
            },
            ...
            substrAmount(amount){
                ...
                if(tokenValue && tokenValue.toString().length > 12){
                    let result =  Tools.formatString(amount.toString(),12,'...');
                    return result;
                }else {
                    return amount
                }
