<template>
  <div :class="echartsComponentWrapLine">
    <div class="echarts_title_wrap_line">
      <span class="echarts_title"><i class="iconfont iconTransactionHistory"></i>{{$t('overview.14daysTransactionHistory')}}</span>
      <span class="view_all_content"><router-link @click.native="$uMeng.push('HomeOverview_14 day Transaction History','click')" :to="`/txs?txType=&status=&startTime=${startTime}&endTime=${endTime}&page=1`">{{$t('overview.viewAll')}}</router-link></span>
    </div>
    <div id="echarts_line">
    ...
    methods: {
      ...
      setThemeStyle(){
        if(this.$store.state.currentSkinStyle ===  Constant.CHAINID.IRISHUB){
          ...
          this.skinStyle = '#F18E00';
          this.lineSkinBackgroundStyle = [
            {offset: 0, color: 'rgba(241,142,0, 0.8)'},
            {offset: 1, color: 'rgba(241,142,0, 0)'}
          ]
        }
      }
    },
