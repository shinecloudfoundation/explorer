                    ...
                    <div class="header_left_content">
                        ...
                        <ul class="header_menu_content">
                            <li class="header_menu_item"
                                ...
                                @mouseenter="showTwoMenu('blockChain')" @mouseleave="hideTwoMenu('blockChain')">{{$t('overview.BlockChain.Header')}}</li>
                            <li class="header_menu_item"
                                :class="activeStaking ? 'nav_item_active' : ''"
                                @mouseenter="showTwoMenu('staking')" @mouseleave="hideTwoMenu('staking')">{{$t('overview.Staking.Header')}}</li>
                            <li class="header_menu_item"
                                :class="activeTransfers ? 'nav_item_active' : ''"
                                @mouseenter="showTwoMenu('transfers')" @mouseleave="hideTwoMenu('transfers')">
                                <router-link :to="`/txs/transfers`">{{$t('overview.Transfer.Header')}}</router-link></li>
<!--                            <li class="header_menu_item"-->
...
<!--                                <router-link :to="`/faucet`">Faucet</router-link></li>-->
                    ...
                    <div class="header_right_content">
                        ...
                        <div class="network_container" @mouseenter="showNetWorkLogo()" @mouseleave="hideNetWorkLogo()">
                            <span :style="{'color': '#ffffff', 'font-size': '1em'}" v-text="currentLanguage"></span>
                            <ul class="network_list_container" v-show="flShowNetworkLogo" @mouseenter="showNetWorkLogo()" @mouseleave="hideNetWorkLogo()">
                                <li :style="{'color': '#0f0f0f', 'font-size': '1em'}" class="network_list_item"
                                    v-for="item in languageArray"
                                    @click="setLanguage(item)" v-text="getLanguage(item)"></li>
                            </ul>
                        </div>
                    </div>
                ...
                <div class="header_submenu_container" @mouseenter="showTwoMenu(menuActiveName)" @mouseleave="hideTwoMenu(menuActiveName)">
                    <div class="header_submenu_content_wrap">
                        <ul class="header_submenu_content"  :style="{'left':offSetLeft,width:contentWidth}">
                            <li class="header_submenu_item" v-show="flShowChain"><router-link :to="`/home`">{{$t('overview.BlockChain.Overview')}}</router-link></li>
                            <li class="header_submenu_item" v-show="flShowChain"><router-link :to="`/blocks`">{{$t('overview.BlockChain.Blocks.Header')}}</router-link></li>
                            <li class="header_submenu_item" v-show="flShowChain"><router-link :to="`/txs`">{{$t('overview.BlockChain.Transactions.Header')}}</router-link></li>
                            ...
                            <li class="header_submenu_item" v-show="flShowStaking"><router-link :to="`/validators`">{{$t('overview.Staking.Validators')}}</router-link></li>
                            <li class="header_submenu_item" v-if="flShowStaking"><router-link :to="`/txs/delegations`">{{$t('overview.Staking.DelegationTxs')}}</router-link></li>
                            <li class="header_submenu_item" v-show="flShowStaking"><router-link :to="`/txs/validations`">{{$t('overview.Staking.ValidatorTxs')}}</router-link></li>
...
<!--                </div>-->
                ...
                <div class="mobile_menu_container" @click="flShowBlockchain('language')">
                    <div class="mobile_menu_item_content">
                        <span :style="{'color': '#ffffff', 'font-size': '1em'}" v-text="currentLanguage" ></span>
                        <i class="iconfont iconwangluoqiehuanjiantou" :class="flShowLanguage ? 'up_style' : 'down_style'"> </i>
                    </div>
                    <ul class="blockchain_list_content" v-show="flShowLanguage">
                        <li class="blockchain_list_item" :style="{'color': '#ffffff', 'font-size': '1em'}" @click="setLanguage(`en`)">English</li>
                        <li class="blockchain_list_item" :style="{'color': '#ffffff', 'font-size': '1em'}" @click="setLanguage(`zhs`)">中文简体</li>
                    </ul>
                </div>
            </div>
        </div>
	...
	export default {
		...
		methods: {
			flShowBlockchain(v){
				switch (v) {
					...
					case 'faucet' :
						...
						break;
                    case 'language' :
                        this.flShowLanguage = !this.flShowLanguage;
                        break;
				}
            },
			showTwoMenu(v){
			...
			getData () {
				if (Tools.removeAllSpace(this.searchInputValue) === '') {
					...
					if (/^[A-F0-9]{64}$/.test(this.searchInputValue)) {
                        ...
                        this.searchHashLock()
                    } else if (this.$Codec.Bech32.isBech32("scloud", this.searchInputValue)) {
						this.searchDelegator();
					} else if (this.$Codec.Bech32.isBech32("scloudvaloper", this.searchInputValue)) {
            ...
            setLanguage (lang) {
                ...
                this.currentLanguage=localeStrings[lang];
            },
            getLanguage (lang) {
			...
			toggleTestnetLogo (currentEnv) {
				...
				if (networkName === constant.CHAINID.IRISHUB) {
                    ...
                    root.style.setProperty(skinStyle.skinStyle.LINKCOLOR,skinStyle.skinStyle.DEFAULTLINKCOLOR);
                }
			},
